...
"""MY-V1 clustering pipeline and unified Benchmark adapter."""
...
class MYV1Result:
    """Complete result and inspectable intermediate state of one MY-V1 run."""
...
def run_algorithm(
    X: np.ndarray,
    n_clusters: int,
    config: MYV1Config | None = None,
    seed: int | None = None,
) -> MYV1Result:
    """Conventional functional entry point used by direct tests."""

    if config is None:
        raise ValueError("MY-V1 requires an explicit MYV1Config(p1=..., p2=...)")
    return run_my_v1(X, n_clusters, config, seed=seed)
...
def run_my_v1(
    ...
    """Run the complete MY-V1 clustering pipeline.
...
class MYV1(BenchmarkAlgorithm):
    """Unified algorithm adapter for MY-V1."""
    ...
    def fit(self, X: np.ndarray) -> "MYV1":
        result = run_my_v1(
            X,
            self.n_clusters,
            self.config,
