...
"""MY-V4 clustering pipeline and unified Benchmark adapter."""
...
class MYV4Result:
    """Complete result and inspectable intermediate state of one MY-V4 run."""

    labels: np.ndarray
    pseudo_labels: np.ndarray
    pseudo_label_confidence: np.ndarray
...
def run_algorithm(
    X: np.ndarray,
    n_clusters: int,
    config: MYV4Config | None = None,
    seed: int | None = None,
) -> MYV4Result:
    """Conventional functional entry point used by direct tests."""

    if config is None:
        raise ValueError("MY-V4 requires an explicit MYV4Config(p1=..., p2=...)")
    return run_my_v4(X, n_clusters, config, seed=seed)
...
def run_my_v4(
    ...
    """Run the complete MY-V4 clustering pipeline.
    ...
    if precomputed_pseudo_labels is None:
        pseudo_labels, centers, weights = weighted_kmeans(
            scaled,
            n_clusters,
    ...
    confidence = _pseudo_label_confidence(
        scaled,
        pseudo_labels,
        centers if precomputed_pseudo_labels is None else None,
    )
...
def _pseudo_label_confidence(
    ...
    """Return a deterministic nearest-vs-second-nearest cluster margin."""

    values = np.asarray(X, dtype=float)
    pseudo = np.asarray(labels, dtype=int).reshape(-1)
    unique = np.unique(pseudo)
    if centers is None:
        centers = np.vstack(
            [
                values[pseudo == label].mean(axis=0)
                if np.any(pseudo == label)
    ...
    return np.clip((second - nearest) / (second + nearest + 1e-8), 0.0, 1.0)


class MYV4(BenchmarkAlgorithm):
    """Unified algorithm adapter for MY-V4."""
    ...
    def fit(self, X: np.ndarray) -> "MYV4":
        result = run_my_v4(
            X,
            self.n_clusters,
            self.config,
        ...
        self.pseudo_label_confidence_ = result.pseudo_label_confidence
