...
"""Granular-ball division used by MY-V4."""
...
def split_ball_with_2means(
    ...
    """Split one ball after MY-V4 pseudo-label-guided local reduction."""

    if ball.size < 2:
        return ball, GranularBall(
            ball.X[:0].copy(), ball.pseudo_labels[:0].copy(), ball.confidence[:0].copy()
        )
    ...
    split_X, _, _ = select_local_features_by_gaussian_pdmf_graph(
        ...
        pseudo_labels=ball.pseudo_labels,
    ...
    if not np.any(first) or not np.any(second):
        ...
        second = ~first
    return (
        GranularBall(ball.X[first], ball.pseudo_labels[first], ball.confidence[first]),
        GranularBall(ball.X[second], ball.pseudo_labels[second], ball.confidence[second]),
    )
...
def generate_granular_balls(
    ...
    if conf.size != values.shape[0]:
        raise ValueError("confidence length must match X")
    balls = [GranularBall(values, pseudo, conf)]
